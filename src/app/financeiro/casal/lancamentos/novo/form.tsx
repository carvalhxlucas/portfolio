...
export default function NovoLancamentoCasalForm({ coupleId, myEmail, partnerEmail }: Props) {
  const router = useRouter()
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [tipo, setTipo] = useState<'receita' | 'despesa'>('despesa')
  const [pagoPor, setPagoPor] = useState(myEmail)
  const [dividir, setDividir] = useState(true)
  const [divisaoPct, setDivisaoPct] = useState(50)
